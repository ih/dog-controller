...
vec3 NormalIntensity(inout vec3 normalTS, float normalIntensity)
{
    normalTS = mix(vec3(0.5, 0.5, 1.0), normalTS, vec3(normalIntensity));
    return normalTS;
}

vec3 DecodeNormal(inout vec3 normalTS, float normalIntensity)
{
    vec3 param = normalTS;
    float param_1 = normalIntensity;
    vec3 _265 = NormalIntensity(param, param_1);
    normalTS = _265;
...
vec3 SamplerEncodedPanoramic(mediump sampler2D panoramic, vec3 dir, float rotation, float lod)
{
    ...
    vec2 _1259 = GetPanoramicTexCoordsFromDir(param, param_1);
    vec2 uv = _1259;
    vec2 param_2 = uv;
    float param_3 = lod;
...
void main()
{
    ...
    float ao = 1.0;
    mat3 TBN = mat3(vec3(v_tDirWS), vec3(v_bDirWS), vec3(v_nDirWS));
    vec3 clearCoatNormal = vec3(0.0, 0.0, 1.0);
    vec3 normal = vec3(0.0, 0.0, 1.0);
    vec3 normalTS = texture(_NormalTexture, uv0).xyz;
    vec3 param_2 = normalTS;
    float param_3 = _NormalIntensity;
    vec3 _2159 = DecodeNormal(param_2, param_3);
