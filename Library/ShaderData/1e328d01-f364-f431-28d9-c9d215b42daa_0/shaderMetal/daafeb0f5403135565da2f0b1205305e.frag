...
static inline __attribute__((always_inline))
float3 NormalIntensity(thread float3& normalTS, thread const float& normalIntensity)
{
    normalTS = mix(float3(0.5, 0.5, 1.0), normalTS, float3(normalIntensity));
    return normalTS;
}

static inline __attribute__((always_inline))
float3 DecodeNormal(thread float3& normalTS, thread const float& normalIntensity)
{
    float3 param = normalTS;
    float param_1 = normalIntensity;
    float3 _265 = NormalIntensity(param, param_1);
    normalTS = _265;
    normalTS = normalize((normalTS * 2.0) - float3(1.0));
    return normalTS;
}
...
float3 SamplerEncodedPanoramic(thread const texture2d<float> panoramic, thread const sampler panoramicSmplr, thread const float3& dir, thread const float& rotation, thread const float& lod)
{
    ...
    float2 _1259 = GetPanoramicTexCoordsFromDir(param, param_1);
    float2 uv = _1259;
    float2 param_2 = uv;
    float param_3 = lod;
...
float3 Diffuse_OrenNayar(thread const SurfaceParams& S, thread const LightParams& L)
{
    ...
    if (Cosri >= 0.0)
    {
        _451 = 1.0 / fast::max(ndl, ndv);
    }
    else
    {
        _451 = 1.0;
    }
    float C2 = (((0.449999988079071044921875 * s2) / (s2 + 0.0900000035762786865234375)) * Cosri) * _451;
...
fragment main0_out main0(main0_in in [[stage_in]], constant buffer_t& buffer, texture2d<float> _AlbedoTexture [[texture(0)]], texture2d<float> _NormalTexture [[texture(1)]], texture2d<float> _AmbientTexture [[texture(2)]], sampler _AlbedoTextureSmplr [[sampler(0)]], sampler _NormalTextureSmplr [[sampler(1)]], sampler _AmbientTextureSmplr [[sampler(2)]])
{
    main0_out out = {};
    float2 uv0 = in.v_uv0;
    ...
    float cutoff = buffer.u_Cutoff;
    float metallic = buffer._Metallic;
    float roughness = buffer._Roughness;
    float ao = 1.0;
    float3x3 TBN = float3x3(float3(in.v_tDirWS), float3(in.v_bDirWS), float3(in.v_nDirWS));
    float3 clearCoatNormal = float3(0.0, 0.0, 1.0);
    float3 normal = float3(0.0, 0.0, 1.0);
    float3 normalTS = _NormalTexture.sample(_NormalTextureSmplr, uv0).xyz;
    float3 param_2 = normalTS;
    float param_3 = buffer._NormalIntensity;
    float3 _2159 = DecodeNormal(param_2, param_3);
    clearCoatNormal = _2159;
    normal = _2159;
    ...
    float3 param_10 = clearCoatNormal;
    ...
    float3 subsurfaceCol;
    float3 param_15 = subsurfaceCol;
    float3 subsurfaceColMultiply;
    float3 param_16 = subsurfaceColMultiply;
    float ior;
    float param_17 = ior;
    float transmittance;
    ...
    float2 param_33 = screen_coord;
    out.glResult = ApplyBlendMode(param_32, param_33);
    return out;
}
